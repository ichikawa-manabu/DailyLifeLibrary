...
public class Initialization {
    ...
    //検索したいsheet_numberを入力
    // 返し値:時刻別仕事をしている人の割合
    public static int[] standard_work_time(File file, int sheet_number) throws IOException {
    ...
    ///////////////////////////
    public static byte[] generate_workperiod_seed(int start_time,int back_time) {
        byte[] genes= new byte[defaultGeneLength];
        int i=0;
        //start_time=(end_time-move_time)%defaultGeneLength;
        if(back_time==0) {//移動時間は１５MIN単位で取ってるので　15分以内なら　今の時間を開始時間にする　移動時間は０
            genes[(start_time+defaultGeneLength)%defaultGeneLength] = 1;
        }

        while(i<(back_time-start_time+defaultGeneLength)%defaultGeneLength){//
    ...
    public static int[] generateworkPopulation(int[] start_time,int[] back_time) {
        byte[] genes = new byte[defaultGeneLength];
        int[] Population = new int[defaultGeneLength];
        for(int i=0;i<NO_OF_PARAMETERS;i++) {
            genes = generate_workperiod_seed(start_time[i], back_time[i]);
            //System.out.println("start_time " +start_time+"and"+i);
